...
- 编写带可选依赖组的 `pyproject.toml`，并生成 lockfile 以确保可复现性
- 诊断并修复常见陷阱：全局安装、pip/conda 混用、CUDA 版本不匹配
- 为存在冲突依赖的项目实现按阶段隔离的环境策略

## 问题

你为微调项目安装了 PyTorch 2.4。下周，另一个项目需要 PyTorch 2.1，因为它的 CUDA 构建是固定的。你全局升级，第一个项目就崩了。你降级，第二个项目又崩了。

这就是依赖地狱。在 AI/ML 工作中它不断发生，因为：
...
一条规则：如果你用 conda 管理某个环境，就用 conda 管理该环境中的所有包。在 conda 环境中混用 `pip install` 会导致痛苦的依赖冲突。

### 本课程策略：按阶段隔离

你可以为整个课程创建一个环境。不要这么做。不同阶段需要不同（有时冲突）的依赖。

策略：

```
ai-engineering-from-scratch/
├── .venv/                    <-- 第 0-3 阶段共享的轻量环境
├── phases/
│   ├── 04-neural-networks/
│   │   └── .venv/            <-- PyTorch 环境
│   ├── 05-cnns/
...
将 lockfile 提交到 git。当有人克隆仓库时，他们从 lockfile 安装并得到完全一致的版本。
...
如果你必须在 conda 中使用 pip（某些包只有 pip 版），先安装所有 conda 包，最后安装 pip 包。
...
虚拟环境有 200MB-2GB。它们是本地的，不能跨机器移植。提交 `pyproject.toml` 和 lockfile 即可。
...
| Lockfile | "固定依赖" | 列出每个包及其精确版本的文件，保证跨机器安装结果一致 |
| pyproject.toml | "新的 setup.py" | 标准的 Python 项目配置文件，替代 setup.py/setup.cfg/requirements.txt |
| 传递依赖（Transitive dependency） | "依赖的依赖" | 包 B 依赖 C；如果你安装 A，而 A 依赖 B，那么 C 就是 A 的传递依赖 |
| CUDA 不匹配 | "我的 GPU 不工作" | PyTorch 编译时使用的 CUDA 版本与 GPU 驱动支持的版本不同 |
