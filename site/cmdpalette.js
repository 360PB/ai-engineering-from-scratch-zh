 ...
 *   CmdPalette.打开()   — 打开 the palette
 *   CmdPalette.关闭()  — 关闭 the palette
 *
...
(function () {
  ...
  // ── Palette DOM (created lazily on first 打开) ────────────────────────
  function createPaletteDOM() {
    ...
    document.getElementById('cpBackdrop').addEventListener('click', 关闭);
    document.getElementById('cpKbdEsc').addEventListener('click', 关闭);
  ...
  function 打开() {
    if (_isOpen) {
      // Already 打开 — make sure the input is focused
      var inp = _inputEl();
      if (inp) inp.focus();
      return;
    }
    ...
    requestAnimationFrame(function () {
      var pal = _palEl();
      if (pal) pal.classList.add('cp-打开');
  ...
  function 关闭() {
    ...
    var pal = _palEl();
    if (pal) pal.classList.remove('cp-打开');
  ...
  document.addEventListener('keydown', function (e) {
    if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
      e.preventDefault();
      if (_isOpen) {
        // Palette is already 打开 — just refocus the input
        var inp = _inputEl();
        if (inp) inp.focus();
      } else {
        打开();
  ...
  function _init() {
    // Any element with [data-cmd-palette] 打开s the palette on click
