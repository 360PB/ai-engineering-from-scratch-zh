  ...
  function initModal() {
    ...
    if (resetBtn) {
      resetBtn.addEventListener('click', function () {
        ...
        var ok = window.confirm('清除所有本地进度（测验答案和已完成的课程）？此操作不可撤销。');
        if (!ok) return;
  ...
  function renderModalLessons(p) {
    ...
    for (var i = 0; i < p.lessons.length; i++) {
      ...
      if ((l.status === 'complete' || userComplete) && lessonPath) {
        actionHtml = '<a href="https://raw.giteeusercontent.com/qianchilang/ai-engineering-from-scratch-zh/raw/master/site/' + lessonPath + '/docs/zh.md" target="_blank" rel="noopener" class="modal-lesson-read">' + (userComplete ? '复习' : '阅读') + '</a>';
      }
      var toggleHtml = '';
