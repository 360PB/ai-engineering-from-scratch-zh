# Julia 优化器。GradientDescent、SGD+Momentum、Adam
# 实现为带有公共 `step!` 方法的可变结构体。
...
function demo_learning_rate_effect()
    ...
    for lr in rates
        ...
        status = diverged ? "发散" : (loss < 0.01 ? "收敛" : "慢")
        if diverged
